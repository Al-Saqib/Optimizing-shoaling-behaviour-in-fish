...
def run_simulation_with_metrics(genome, num_fish):
    ...
    cohesion_values, separation_values, alignment_values = [], [], []
    ...
    return len(fishes), cohesion_values, separation_values, alignment_values



def run_random_search_for_population_size(num_fish, runs=5, generations=10, population_size=16):
...
def run_multiple_simulations(genome, num_fish, num_simulations=10):
    all_cohesion, all_separation, all_alignment = [], [], []

    for _ in range(num_simulations):
        _, cohesion_values, separation_values, alignment_values = run_simulation_with_metrics(genome, num_fish)
        all_cohesion.extend(cohesion_values)
        all_separation.extend(separation_values)
        all_alignment.extend(alignment_values)

    metrics = {
        'cohesion': (np.mean(all_cohesion), np.std(all_cohesion)),
        'separation': (np.mean(all_separation), np.std(all_separation)),
        'alignment': (np.mean(all_alignment), np.std(all_alignment))
    }
    return metrics
...
fish_population_sizes = range(10, 51, 10)
...
for idx, num_fish in enumerate(fish_population_sizes):
    avg_traits = best_results[idx][0]
    avg_genome = Genome(**avg_traits)
    metrics = run_multiple_simulations(avg_genome, num_fish)
    metrics_per_population_size.append(metrics)



# Plotting Best Average Survival Rates
...
# Plotting Behavioral Metrics
plt.figure(figsize=(15, 5))
for i, metric in enumerate(['cohesion', 'separation', 'alignment']):
    metric_means = [m[metric][0] for m in metrics_per_population_size]
    metric_stds = [m[metric][1] for m in metrics_per_population_size]
    plt.subplot(1, 3, i + 1)
    plt.errorbar(fish_population_sizes, metric_means, yerr=metric_stds, fmt='o-', capsize=5)
    plt.title(f'{metric.capitalize()} vs Fish Population Size')
    plt.xlabel('Fish Population Size')
