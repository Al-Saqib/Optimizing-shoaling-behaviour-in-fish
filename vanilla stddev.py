...
def run_genetic_algorithm_for_population_size(num_fish, runs=5, generations=10, population_size=16):
...
def run_multiple_simulations_with_metrics(genome, num_fish, num_simulations=10):
    all_cohesion, all_separation, all_alignment = [], [], []

    for _ in range(num_simulations):
        _, cohesion, separation, alignment = run_simulation_with_metrics(genome, num_fish)
...
for idx, num_fish in enumerate(fish_population_sizes):
    ...
    metrics = run_multiple_simulations_with_metrics(avg_genome, num_fish)
...
# Plotting the Behavioral Metrics with Mean and Standard Deviation
plt.figure(figsize=(15, 5))
metrics = ['cohesion', 'separation', 'alignment']  # Lowercase to match the dictionary keys
for i, metric in enumerate(metrics):
    metric_means = [result[metric][0] for result in metrics_per_population_size]
    metric_stds = [result[metric][1] for result in metrics_per_population_size]
